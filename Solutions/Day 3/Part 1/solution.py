...
def parse_instruction(instruction):
    """Determine direction and distance of instruction."""
    direction = instruction[0]
    distance = int(instruction[1:])
    parsed_instruction = (direction, distance)

    return parsed_instruction

def do_instruction(wire, direction, distance):
    """Do an instruction."""
    if direction == 'R':
        wire[0] += distance
    elif direction == 'L':
...
for i in range(1000):
    for wire in wires:
        w_index= wires.index(wire)
        if w_index == 0:
            w_loc = wire_1_loc
        else:
            w_loc = wire_2_loc

        direction, distance = parse_instruction(wire[i])
        do_instruction(w_loc, direction, distance)

    if wire_1_loc[0] == wire_2_loc[0] and wire_1_loc[1] == wire_2_loc[1]:
        x = wire_1_loc[0]
        y = wire_2_loc[1]
